...
def handle_create(data):
    ...
    if name and name not in ROOMS:
        ROOMS[name] = {"history": [], "temp": data.get('temp', False), "pass": data.get('password', ""), "users": set()}
        sync_rooms()

@socketio.on('join')
def handle_join(data):
    room, pw, old = data.get('room'), data.get('password', ""), data.get('old_room')
    if room in ROOMS:
        ...
        if old in ROOMS:
            leave_room(old)
            if request.sid in ROOMS[old]["users"]: ROOMS[old]["users"].remove(request.sid)
        join_room(room)
